...
void agregarMedicamentosAleatoriamente(int numMedicamentos) {
    for (int i = 0; i < numMedicamentos; i++) {
        medicamento m;
        m.nombre = "Medicamento" + to_string(i);
        m.precio = rand() % 1000 + 1; // Precio aleatorio entre 1 y 1000
        m.cantidad = rand() % 100 + 1; // Cantidad aleatoria entre 1 y 100
        listaMedicamentos.push_back(m.nombre);
        medicamentos[m.nombre] = m;
    }
}

// Función para agregar medicamentos repetidamente
void agregarMedicamentos(int numVeces) {
    
    auto inicio = chrono::high_resolution_clock::now();

    for (int i = 0; i < numVeces; i++) {
        agregarMedicamentosAleatoriamente(1); // Agregamos un medicamento en cada iteración
    }
    ...
    cout << "Tiempo para agregar " << numVeces << " medicamentos: " << tiempo.count() << " microsegundos" << endl;
    /*
    // Guardar el tiempo en el archivo de datos
    ofstream archivoTiempos("tiempos_agregar_medicamentos.txt", ios::app);
    if (archivoTiempos) {
        archivoTiempos << numVeces << " " << tiempo.count() << endl;
        archivoTiempos.close();
    } else {
        cerr << "Error al guardar el tiempo en el archivo" << endl;
    }
    */
}

void VerDetalleDeMedicamento(int a){
  for(int i = 1; i <= listaMedicamentos.size(); i++){
    cout << i << ". " << listaMedicamentos[i-1] << endl;
  }
  if(a > listaMedicamentos.size() || a < 1){
    cout << "Opcion invalida" << endl;
    return;
  }
  cout << "Nombre: "<< medicamentos[listaMedicamentos[a-1]].nombre << endl;
  cout << "Precio: "<< medicamentos[listaMedicamentos[a-1]].precio << endl;
  cout << "Stock: "<< medicamentos[listaMedicamentos[a-1]].cantidad << endl;
}
bool venderUnMedicamento(int opcion) {
  int cantidad = 1; 
  cout << "Medicamentos" << endl;
  for (int i = 1; i <= listaMedicamentos.size(); i++) {
    cout << i << ". " << listaMedicamentos[i - 1] << endl;
...
double medirTiempoAgregandoMedicamentos(int n) {
    auto start = chrono::high_resolution_clock::now();

    agregarMedicamentos(n);

    auto end = chrono::high_resolution_clock::now();
    chrono::duration<double> duration = end - start;
    return duration.count(); // Devolver el tiempo en segundos
}

double medirTiempovendiendoUnMedicamento(int n) {
    auto start = chrono::high_resolution_clock::now();

    venderUnMedicamento(n);
...
double medirTiempoBuscandoMedicamento(int n) {
    auto start = chrono::high_resolution_clock::now();

    VerDetalleDeMedicamento(n);
...
double medirTiempoMostrandoMedicamentos(int n) {
    auto start = chrono::high_resolution_clock::now();
...
void generarGraficoIndividuales() {
    FILE *gnuplotPipe = popen("gnuplot -persist", "w");
    if (gnuplotPipe) {
        // Configurar gnuplot para generar la gráfica individual
        fprintf(gnuplotPipe, "set title 'Tiempo de ejecucion vs Numero de medicamentos'\n");
        fprintf(gnuplotPipe, "set xlabel 'Numero de medicamentos'\n");
        fprintf(gnuplotPipe, "set ylabel 'Tiempo (segundos)'\n");
        
        //fprintf(gnuplotPipe, "plot 'tiempos_individuales_agregar_medicamentos.txt' with linespoints title 'Tiempo individual de ejecucion'\n");
        //fprintf(gnuplotPipe, "plot 'tiempos_individuales_buscar_medicamentos.txt' with linespoints title 'Tiempo individual de ejecucion'\n");
        //fprintf(gnuplotPipe, "plot 'tiempos_individuales_detalles_medicamentos.txt' with linespoints title 'Tiempo individual de ejecucion'\n");
        fprintf(gnuplotPipe, "plot 'tiempos_individuales_vender_medicamentos.txt' with linespoints title 'Tiempo individual de ejecucion'\n");
        fprintf(gnuplotPipe, "quit\n");
        pclose(gnuplotPipe);
    } else {
        cerr << "Error al invocar gnuplot. Asegúrate de tener gnuplot instalado en tu sistema." << endl;
    }
}

void generarGraficoAcumulados() {
  //-----Archivos Individuales-------- 
  //(los acumulados dependen de esots ----> seleccionar el correcto)
  
  // Leer los tiempos individuales desde el archivo tiempos_individuales_agregar_medicamentos.txt
  //ifstream archivoIndividuales("tiempos_individuales_agregar_medicamentos.txt");
  
  //ifstream archivoIndividuales("tiempos_individuales_buscar_medicamentos.txt");

  //ifstream archivoIndividuales("tiempos_individuales_detalles_medicamentos.txt");

  ifstream archivoIndividuales("tiempos_individuales_vender_medicamentos.txt");
  if (!archivoIndividuales.is_open()) {
      cerr << "Error al abrir el archivo tiempos_individuales_agregar_medicamentos.txt" << endl;
      return;
  }

  // Vector para almacenar los tiempos individuales
  vector<double> tiemposIndividuales;
  double tiempoAcumulado = 0.0;
  double tiempo;
  while (archivoIndividuales >> tiempo) {
      tiempoAcumulado += tiempo;
      tiemposIndividuales.push_back(tiempoAcumulado);
  }
  archivoIndividuales.close();


  //-------FUNCION 1--------
  //archivo para el tiempo de agregarMedicamento()
  //ofstream archivoAcumulados("tiempos_acumulados_agregar_medicamentos.txt");
  //-------FUNCION 2--------
  //archivo para el tiempo de buscarMedicamento()
  //ofstream archivoAcumulados("tiempos_acumulados_buscar_medicamentos.txt");

  //-------FUNCION 3--------
  //archivo para el tiempo de verDetallesMedicamento()
  //ofstream archivoAcumulados("tiempos_acumulados_detalles_medicamentos.txt");
  
  //-------FUNCION 3--------
  //archivo para el tiempo de verDetallesMedicamento()
  ofstream archivoAcumulados("tiempos_acumulados_vender_medicamentos.txt");
  
  
  if (archivoAcumulados.is_open()) {
      for (double tiempoAcum : tiemposIndividuales) {
          archivoAcumulados << tiempoAcum << endl;
      }
      archivoAcumulados.close();
  } else {
      cerr << "Error al abrir el archivo tiempos_acumulados_agregar_medicamentos.txt" << endl;
  }

  FILE *gnuplotPipe = popen("gnuplot -persist", "w");
  if (gnuplotPipe) {
      // Configurar gnuplot para generar la gráfica acumulada
      fprintf(gnuplotPipe, "set title 'Tiempo acumulado vs Numero de medicamentos'\n");
      fprintf(gnuplotPipe, "set xlabel 'Numero de medicamentos'\n");
      fprintf(gnuplotPipe, "set ylabel 'Tiempo acumulado (segundos)'\n");
      
      //fprintf(gnuplotPipe, "plot 'tiempos_acumulados_agregar_medicamentos.txt' with linespoints title 'Tiempo acumulado'\n");
      //fprintf(gnuplotPipe, "plot 'tiempos_acumulados_buscar_medicamentos.txt' with linespoints title 'Tiempo acumulado'\n");
      //fprintf(gnuplotPipe, "plot 'tiempos_acumulados_detalles_medicamentos.txt' with linespoints title 'Tiempo acumulado'\n");
      fprintf(gnuplotPipe, "plot 'tiempos_acumulados_vender_medicamentos.txt' with linespoints title 'Tiempo acumulado'\n");
      fprintf(gnuplotPipe, "quit\n");
      pclose(gnuplotPipe);
  } else {
      cerr << "Error al invocar gnuplot. Asegúrate de tener gnuplot instalado en tu sistema." << endl;
...
int main() {
  ...
  //Para las pruebas se trabajan con 20 Medicamentos generados aleatoriamente
  agregarMedicamentosAleatoriamente(50);
  
  //-------FUNCION 1--------
  //archivo para el tiempo de agregarMedicamento()
  //ofstream archivoIndividuales("tiempos_individuales_agregar_medicamentos.txt");


  //-------FUNCION 2--------
  //archivo para el tiempo de buscarMedicamento()
  //ofstream archivoIndividuales("tiempos_individuales_buscar_medicamentos.txt");

  //-------FUNCION 3--------
  //archivo para el tiempo de verDetallesMedicamento()
  //ofstream archivoIndividuales("tiempos_individuales_detalles_medicamentos.txt");

  
  //-------FUNCION 3--------
  //archivo para el tiempo de verDetallesMedicamento()
  ofstream archivoIndividuales("tiempos_individuales_vender_medicamentos.txt");

  if (archivoIndividuales.is_open()) {
      for (int i = 1; i <= 50; i++) {
        //Medir tiempo usando la funcion AgregandoMedicamento
        //double tiempoEjecucion = medirTiempoAgregandoMedicamentos(i);

        
        //Medir tiempo usando la funcion Mostrando Medicamentos
        //double tiempoEjecucion = medirTiempoMostrandoMedicamentos(i);
        
        //Medir tiempo usando la funcion Mostrando Medicamentos
        //double tiempoEjecucion = medirTiempoBuscandoMedicamento(i);
        
        //Medir tiempo usando la funcion Mostrando Medicamentos
        double tiempoEjecucion = medirTiempovendiendoUnMedicamento(i);
        archivoIndividuales << tiempoEjecucion << endl;
  ...
  generarGraficoIndividuales();

  generarGraficoAcumulados();
